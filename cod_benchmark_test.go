...
func Benchmark_CanadianOpenData(b *testing.B) {
	...
	for _, numH := range numHash {
		...
		// Output results
		accuracy_filename := fmt.Sprintf("_cod_accuracy_siphash_64_%d_large.csv", numH)
		...
		performance_filename := fmt.Sprintf("_cod_performance_siphash_64_%d_large.csv", numH)
